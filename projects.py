...
def import_project(project):
    ...
    exported = False
    while not exported:
        presigned_get_url = generate_presigned_url(name, "GET")
        if presigned_get_url is not None:
            exported = True
            break
        time.sleep(0.5)
    
    return import_from_s3(name, namespace, presigned_get_url)
...
def migrate_project_info():
    ...
    for project in projects:
        ...
        print json.dumps(new_project)
        print json.dumps(new_project[0])
