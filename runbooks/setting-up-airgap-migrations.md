...
- Set an environment variable called `$CONGREGATE_PATH` to point to a location where you want to store any data from the container
...
- In `$CONGREGATE_PATH`, edit or create the `congregate.conf` file to match the configurations below. Update any paths or URLs accordingly
