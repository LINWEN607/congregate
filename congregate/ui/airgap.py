...
def trigger_export():
    ...
    project = safe_json_response(ProjectsApi().get_project(
        payload.pid, payload.host, payload.token))
...
def trigger_import():
    ...
    payload = AirgapImportPayload(host=formdata.get(
        'host'), token=formdata.get('token'), gid=formdata.get('gid'))
    group = safe_json_response(GroupsApi().get_group(
        payload.gid, payload.host, payload.token))
    ...
    if file and allowed_file(file.filename):
        ...
        result = import_task.delay(
            upload_path, group, payload.host, payload.token)
        return jsonify({
