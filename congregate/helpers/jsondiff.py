"""
JsonDiff: Library for JSON DIFF - based on opslib jsondiff
-------------------------------
...
class Comparator(object):
    ...
    def _compare_arrays(self, old_arr, new_arr):
        ...
        # old_arr_dict = list_to_dict(old_arr)
        # new_arr_dict = list_to_dict(new_arr)

        for idx in range(len(old_arr)):
            if old_arr[idx] not in new_arr:
                result[idx] = {u'---': old_arr[idx]}
        for idx in range(len(new_arr)):
            if new_arr[idx] not in old_arr:
        ...
        # return self.compare_dicts(old_arr_dict, new_arr_dict)
