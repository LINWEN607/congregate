...
def sign_in(csrf, cookies):
    ...
    url = r.url
    print url
    if "?reset_password_token=" in url:
        data = {
            "utf8": "✓",
            "user[password]": "password",
            "user[password_confirmation]": "password",
            "user[reset_password_token]": url.split("?reset_password_token=")[1]
        }
        r = requests.put(password_route, data=data, cookies=cookies)
        print "Signing in again"
        sign_in(csrf, cookies)
    print r.text
    token = find_csrf_token(r.text)
