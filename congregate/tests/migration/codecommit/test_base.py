...
class TestCodeCommitApiWrapper(unittest.TestCase):
    ...
    @patch('congregate.helpers.conf.Config.src_aws_secret_access_key', new_callable=PropertyMock)
    @patch.object(ConfigurationValidator, 'validate_codecommit_credentials')
    def setUp(self, validate_credentials, mock_secret_access_key, mock_src_aws_region, mock_source_type):
        """Set up a new instance of CodeCommitApiWrapper with mocks for Boto3 client."""
        validate_credentials.return_value = True
        self.mock_boto_client = MagicMock()
        mock_secret_access_key.return_value = "AKIA1234"
