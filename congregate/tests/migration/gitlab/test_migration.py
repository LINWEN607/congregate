...
class MigrationEndToEndTest(unittest.TestCase):
    ...
    def test_project_migration_diff(self):
        project_diff = ProjectDiffClient(
            "%s/data/project_migration_results.json" % self.b.app_path)
        diff_report = project_diff.generate_project_diff_report()
        project_diff.generate_html_report(
            diff_report, "%s/data/project_migration_results.html" % self.b.app_path)
        self.assertGreater(
