...
class MigrationEndToEndTestSetup(unittest.TestCase):
    def setUp(self):
        ...
        self.generate_config_with_tokens()
        self.s = SeedDataGenerator()

    def test_seed_data(self):
        _, groups, _ = self.s.generate_seed_data(dry_run=False)
        group_id = groups[-1]['id']
        self.generate_single_group_config_with_tokens(group_id=str(group_id))

    def generate_config_with_tokens(self):
    ...
    def generate_single_group_config_with_tokens(self, src_parent_group="yes", group_id="1"):
        print("Prepping config data")
        values = [
            os.getenv("GITLAB_DEST"),  # Destination hostname
            # destination_token
            ...
            # src_access_token
            # src_tier
            src_parent_group,  # source parent group
