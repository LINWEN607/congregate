...
class StageWaveCSVGeneratorTests(unittest.TestCase):
    ...
    def test_bad_map_logs_error_and_returns(self, projects, spreadsheet_path):
        ...
        with self.assertLogs() as cm:            
            self.wcli.generate(
                destination_file="generate_test.csv",
    ...
    def test_generate_happy(self, projects, spreadsheet_path, column_mapping, columns_to_use):
        ...
        columns_to_use.return_value = ["Wave name", "Wave date", "Source Url", "Parent Path"]
        ...
        expected = [
            ...
            ["", "", "http://example.com/brightbox/puppet.git", ""] # Odd our project mock returns two of same project?
        ]
        header_info = {
            "headers": ["Wave name", "Wave date", "Source Url", "Parent Path"],
            "header_map": {
        ...
        self.wcli.generate(destination_file="generate_test.csv", header_info=header_info, dry_run=False)
