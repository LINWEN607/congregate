...
class LdapGroupSyncTests(unittest.TestCase):
    ...
    def test_load_file_duplicate_group_id_logs_warning(self):
        with self.assertLogs(self.ldap.log) as cm:
            ...
            self.assertTrue(self.ldap.ldap_dict.get("01234") == "CN=x,OU=y")
            self.assertTrue(self.ldap.ldap_dict.get("56789") == "someCommonName")
        self.assertEqual(cm.output, ["WARNING:congregate.helpers.base_class:group_id 56789 duplicated in file"])
    ...
    @mock.patch.object(Config, 'ldap_group_link_provider', new_callable=mock.PropertyMock)
    @mock.patch.object(Config, 'ldap_group_link_group_access', new_callable=mock.PropertyMock)
    ...
    @mock.patch("congregate.helpers.api.generate_post_request")
    def test_load_config_gets_defaults(self, p):
