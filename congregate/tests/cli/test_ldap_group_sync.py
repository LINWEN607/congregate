...
class LdapGroupSyncTests(unittest.TestCase):
    ...
    @patch('congregate.helpers.conf.Config.destination_host',
           new_callable=PropertyMock)
    @patch.object(Config, 'ldap_group_link_provider',
                  new_callable=PropertyMock)
    @patch.object(Config, 'ldap_group_link_group_access',
                  new_callable=PropertyMock)
    def test_check_log_info_on_dry_run(
            self, ldap_group_link_group_access, ldap_group_link_provider, dstn_hostname, dstn_access_token):
    ...
    def test_load_config_gets_defaults(
            self, p, dstn_hostname, dstn_access_token):
