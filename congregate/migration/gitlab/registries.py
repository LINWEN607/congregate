...
class RegistryClient(BaseClass):
    ...
    def enabled(self):
        src = self.__enabled__(self.config.child_host, self.config.child_token)
        dest = self.__enabled__(self.config.parent_host, self.config.parent_token)
        return src and dest

    def __enabled__(self, host, token):
        return api.generate_get_request(host, token, "projects")["container_registry_enabled"]

    # Get a list of registry repositories in a project
    def __list_registry_repositories__(self, host, token, id):
        return api.list_all(host, token, "projects/%d/registry/repositories" % id)

    # Get a list of tags for given registry repository
    def __list_repository_tags__(self, host, token, id, repo_id):
        return api.list_all(host, token, "projects/%d/registry/repositories/%d/tags" % (id, repo_id))

    # Get details of a registry repository tag
    def __get_repository_tag_details__(self, host, token, id, repo_id, tag_name):
    ...
    def migrate_registries(self, id, old_id):
        try:
            ...
            client = self.__login_to_registry__(self.config.child_host, self.config.child_token, self.config.parent_container_registry_url)
            registries = self.__list_registry_repositories__(self.config.child_host, self.config.child_token, old_id)
            for registry in registries:
                tags = self.__list_repository_tags__(self.config.child_host, self.config.child_token, old_id, registry["id"])
                if list(tags):
                    reg = registry["location"]
                    self.log.info("Pulling images from registry %s" % reg)
                    images = client.images.pull(reg)
                    self.__import_registries__(images, registry)
        except (APIError) as err:
            self.log.error("Failed to export registry, with error:\n%s" % err)

    def __import_registries__(self, images, registry):
        try:
            # Login to destination registry
            client = self.__login_to_registry__(self.config.parent_host, self.config.parent_token, self.config.parent_container_registry_url)
    ...
    def __login_to_registry__(self, host, token, registry_url):
        try:
            client = from_env()
