...
class ImportExportClient(BaseClass):
    ...
    def wait_for_export_to_finish(self, host, token, id):
        ...
        while not exported:
            ...
            if response.status_code == 200:
                ...
                self.log.info("Project doesn't exist. Skipping %s export" % name)
                exported = False
                break
        ...
        if isinstance(project["members"], list):
            for member in project["members"]:
                if project["namespace"] == member["username"]:
                    ...
                    self.log.info("%s is a user project belonging to %s. Attempting to import into their namespace" % (project["name"], new_user))
                    break
            if not user_project:
                self.log.info("%s is not a user project. Attempting to import into a group namespace" % (project["name"]))
                ...
                while not exported:
                            ...
                            if len(search_response) > 0:
                                for proj in search_response:
                                    if proj["name"] == project["name"] and project["namespace"] in proj["namespace"]["path"]:
                                        ...
                                        import_id = proj["id"]
                                        break
                            self.log.info("Project may already exist but it cannot be found. Ignoring %s" % project["name"])
                            return None
                        elif "404 Namespace Not Found" in import_response.get("message"):
                            self.log.info("Skipping %s. Will need to migrate later." % name)
                            import_id = None
                            break
                        elif "The project is still being deleted" in import_response.get("message"):
                            self.log.info("Previous project export has been targeted for deletion. Skipping %s" % project["name"])
                            import_id = None
                            break
                    if import_id is not None:
                        ...
                        if status["import_status"] == "finished":
                            self.log.info("%s has been exported and import is occurring" % name)
                            exported = True
                        ...
                        if timeout < 3600:
                            ...
                            self.log.info("Moving on to the next project. Time limit exceeded")
                            break
        else:
    ...
    def export_import_thru_filesystem(self, id, name, namespace):
        working_dir = getcwd()
        self.log.info("Exporting %s to %s" % (name, self.config.filesystem_path))
    ...
    def export_import_thru_aws(self, id, name, namespace):
        ...
        if not project_exists:
            self.log.info("%s could not be found in parent instance. Exporting project on child instance." % name)
