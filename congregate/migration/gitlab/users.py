...
class gl_users_client(base_class):
    ...
    def remove_users_from_parent_group(self):
        count = 0
        users = api.list_all(self.config.parent_host, self.config.parent_token, "groups/%d/members" % self.config.parent_id)
        for user in users:
            if user["access_level"] <= 20:
                count += 1
                api.generate_delete_request(self.config.parent_host, self.config.parent_token, "/groups/%d/members/%d" % (self.config.parent_id, user["id"]))
            else:
    ...
    def update_user_after_migration(self):
        ...
        for user in users:
            ...
            new_user = api.search(self.config.parent_host, self.config.parent_token, 'users', user['email'])
            print new_user
            if len(new_user) > 0:
                new_users.append(new_user[0])
            else:
                print "searching for %s" % user["username"]
                new_user2 = api.search(self.config.parent_host, self.config.parent_token, 'users', user['username'])
                if len(new_user2) > 0:
    ...
    def retrieve_user_info(self, quiet=False):
        users = list(api.list_all(self.config.child_host, self.config.child_token, "users"))
        root_index = None
        for user in users:
            # Removing root user
            if user["id"] == 1:
