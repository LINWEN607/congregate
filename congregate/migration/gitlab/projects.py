...
class ProjectsClient(BaseClass):
    ...
    def add_shared_groups(self, old_id, new_id):
        ...
        old_project = self.projects_api.get_project(old_id, self.config.child_host, self.config.child_token).json()
        for group in old_project["shared_with_groups"]:
            ...
            if new_group_id is not None:
                ...
                try:
                    r = self.projects_api.add_shared_group(self.config.parent_host, self.config.parent_token, new_id, data)
