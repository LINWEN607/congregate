...
class GroupsClient(BaseClass):
    ...
    def find_parent_group_path(self):
        ...
        try:
            if self.config.parent_id is not None:
                return self.groups_api.get_group(self.config.parent_id, self.config.parent_host, self.config.parent_token).json()["full_path"]
            else:
                return ""
    ...
    def retrieve_group_info(self, host, token, location="source", top_level_group=False, quiet=False):
        ...
        if not top_level_group:
            ...
            if self.config.parent_id is not None:
                groups = [self.groups_api.get_group(self.config.parent_id, self.config.parent_host, self.config.parent_token).json()]
                            ...
                            if response.get("message", None) is not None:
                                if "Failed to save group" in response["message"]:
                                    ...
                                    if group["parent_id"] is not None:
                                        ...
                                        for ng in self.groups_api.search_for_group(group["name"], self.config.parent_host, self.config.parent_token):
                                            if ng["full_path"] == full_parent_namespace:
                                                new_group_id = ng["id"]
                                                print new_group_id
                                                self.log.info("Group found")
    ...
    def make_all_internal_groups_private(self):
        ...
        for group in groups:
            try:
                ...
                for proj in self.groups_api.search_for_group(self.config.parent_host, self.config.parent_token, group['name']):
