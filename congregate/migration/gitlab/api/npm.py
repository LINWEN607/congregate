...
class NpmPackagesApi(GitLabApiWrapper):
    ...
    def upload_npm_package(self, host, token, pid, package_data: NpmPackageData):
        """
        ...
        files, data = package_data.transform_to_multipart()
        self.log.info(f"data to upload: {data}")
        headers = {
            'Private-Token': token,
            'Content-Type': 'application/json'
        }
        
        return self.api.generate_post_request(host, token, f"projects/{pid}/packages/npm/{package_data.name}", files=files, data=data, headers=headers)
