...
def get_gitlab_version(host, token):
...
def get_gitlab_revision(host, token):
...
def create_git_bundle(project_path, output_path, clone_url):
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            repo = Repo.clone_from(clone_url, temp_dir, env={'GIT_SSL_NO_VERIFY': '1'})
        ...
        print(f"Git command error while creating bundle for {project_path}: {str(e)}")
        return False
