...
Arguments:
    ...
    wave                                    Diff between actual and staged data
...
if __name__ == '__main__':
    ...
    WAVE = True if arguments["--wave"] else False
        ...
        if config.external_source_url is not None:
            ...
            if arguments["generate-diff"]:
                user_diff = UserDiffClient(
                    "%s/data/user_migration_results.json" % app_path, staged=WAVE)
                ...
                group_diff = GroupDiffClient(
                    "%s/data/group_migration_results.json" % app_path, staged=WAVE)
                ...
                project_diff = ProjectDiffClient(
                    "%s/data/project_migration_results.json" % app_path, staged=WAVE)
