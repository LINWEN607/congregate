...
def generate_config():
    ...
    shared_runners_enabled = raw_input("Enable shared runners on destination instance (default: True): ")
    ...
    project_suffix = raw_input("Append suffix to project found on destination instance (default: False): ")
    config.set("DESTINATION", "project_suffix",
        "True" if project_suffix and project_suffix.lower() == "true" else "False")
    notification_level = raw_input("Set project/group notification level, disabled (default), participating, watch, global, mention or custom: ")
    ...
    max_import_retries = raw_input("Max no. of project import retries (default: 3): ")
    ...
    ext_src_url = raw_input("Migrating from an external (non-GitLab) instance? Input external source URL: ")
    if ext_src_url and not "gitlab" in ext_src_url.lower():
        ...
        repo_path = raw_input("Absolute path to JSON file containing repo information: ")
    ...
    if not config.has_section("EXT_SRC"):
        ...
        if config.has_option("DESTINATION", "parent_group_id") and config.get("DESTINATION", "parent_group_id"):
            ...
            username_suffix = raw_input("To avoid username collision, please input suffix to append: ")
            config.set("DESTINATION", "username_suffix", username_suffix if username_suffix != "_" else "")

        mirror = raw_input("Planning a soft cut-over migration by mirroring repos to keep both instances running? Yes or No (default): ")
        ...
        location = raw_input("Staging location for exported projects, AWS or filesystem (default): ")
        ...
        abs_path = raw_input("ABSOLUTE path for exporting/updating projects (default: {}): ".format(getcwd()))
        config.set("EXPORT", "filesystem_path", abs_path if abs_path and abs_path.startswith("/") else getcwd())

    # User specific settings
    config.add_section("USER")
    keep_blocker_users = raw_input("Keep blocked users in staged users/groups/projects (default: False): ")
    config.set("USER", "keep_blocked_users",
        "True" if keep_blocker_users and keep_blocker_users.lower() == "true" else "False")
    reset_pwd = raw_input("Users receive password reset emails (default: True): ")
    config.set("USER", "reset_pwd", "False" if reset_pwd and reset_pwd.lower() == "false" else "True")
    force_rand_pwd = raw_input("Users are created with a randomized password (default: False): ")
    ...
    strip_namespace_prefix = raw_input("Strip export/import namespace prefix (default: True): ")
    config.set("APP", "strip_namespace_prefix",
        "False" if strip_namespace_prefix and strip_namespace_prefix.lower() == "false" else "True")
    export_import_wait_time = raw_input("Wait time (in seconds) for project export/import status (default: 30): ")
