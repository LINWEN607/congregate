...
class LdapGroupSync(BaseClass):
    ...
    """
    def __init__(self):
        super(LdapGroupSync, self).__init__()
        self.ldap_dict = {}
        self.ldap_results = []
        ...
        with open(file_path) as ldap_group_mapping_file:
            ...
            for row in csv_reader:
                ...
                if self.ldap_dict.get(group_id, None) is None:
                    ...
                    self.log.warning("group_id %s duplicated in file" % (group_id))
    ...
    def synchronize_groups(self, dry_run=True):
        ...
        for link in self.ldap_dict:
            gitlab_api = "/groups/{group_id}/ldap_group_links".format(group_id=link)
            gitlab_post_data = {
                "provider": self.config.ldap_group_link_provider,
                "cn": self.ldap_dict[link],
                "group_access": self.config.ldap_group_link_group_access
            }
            gitlab_post_description = "Linking {group_id} with {cn}".format(group_id=link, cn=self.ldap_dict[link])
            self.log.info(gitlab_post_description)
            if not dry_run:
                response_json = api.generate_post_request(host=gitlab_host, token=gitlab_token, api=gitlab_api, data=gitlab_post_data, description=gitlab_post_description).json()
