...
- [responses](https://pypi.org/project/responses/0.3.0/)
    - Open source testing library used to intercept HTTP requests generated by the `requests` library and overload the data returned
    - We currently use requests for the following API wrappers
      - Azure DevOps
      - BitBucket Server
      - BitBucket Cloud
      - GitHub
...
- [respx](https://lundberg.github.io/respx/)
    - Testing library used to intercept HTTP requests generated by `httpx` and overload the data returned.
    - Httpx is currently used with our GitLab API wrappers
...
Our integration tests will utilize either the `responses` library or the `respx` library to mock HTTP requests. For example:
...
First let's look at the decorators. 
- `@respx.mock` is similiar to `@responses.activate` in that we are enabling respx to intercept HTTP requests in this test
- We are then patching some data
  - two properties from the base config class
  - one property from the configuration validator class. 
    We are patching the object here because `source_token` makes an API call to ensure what we have set as a source token is a valid token. 
    The two base config values can be patched directly because those properties are just returning data stored in the config file
  - The return value of the `generate_v4_request_url` class function in our GitLab API wrapper. 
    We call this function while making an API request so we can patch it's return value to control the exact URL we are trying to intercept with `respx`

Next let's look at the test function signature. Notice the order of the parameters are inverse of the decorators. So that maps like this:
- `self` - the test class itself
- `url` maps to `@patch("gitlab_ps_utils.api.generate_v4_request_url")`
- `src_token` maps to `@patch.object(ConfigurationValidator, 'source_token', new_callable=PropertyMock)`
- `src_host` maps to `@patch('congregate.helpers.conf.Config.source_host', new_callable=PropertyMock)`
- `src_type` maps to `@patch('congregate.helpers.conf.Config.source_type', new_callable=PropertyMock)`

Notice `@respx.mock` doesn't get passed in through the function parameters, instead we'll interact with it within the test function itself
...
# 2. Adding the specific request we want to mock to respx and providing an expected Response
respx.get(url_value).mock(return_value=Response(
    status_code=404,
    json=self.mock_users.get_user_404()
))

# 3. Running the function we want to test and return its actual value
actual = self.users.find_user_by_email_comparison_with_id(5)

# 4. Calling an assertion to ensure what we are testing matches what we want to test. In this case it's to make sure None is returned
self.assertIsNone(actual)
```

For more examples of integration tests using respx, refer to our GitLab tests located in the codebase at `congregate/tests/migration/gitlab`

Regardless of the HTTP mocking library you use, if you are testing a method making several HTTP requests you will need to mock every request with responses.
